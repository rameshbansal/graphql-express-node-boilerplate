  ...
  return selections.reduce((set, ast) => {
    ...
    switch (ast.kind) {
      case 'Field':
        ...
        if (ast.selectionSet) {
          set[newPrefix] = getFieldSet(info, ast);
          return set;
          set[newPrefix] = {};
        }
      ...
      case 'InlineFragment':
        return Object.assign({}, set, getFieldSet(info, ast));
      case 'FragmentSpread':
        return Object.assign({}, set, getFieldSet(info, info.fragments[ast.name.value]));
    }
  }, {});
}

module.exports = function getFieldList(info) {
  return getFieldSet(info);
};
